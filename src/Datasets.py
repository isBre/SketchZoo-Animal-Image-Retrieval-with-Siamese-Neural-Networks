    ...
    def __getitem__(self, index: int):
        ...
        if should_get_same_class:
            ...
            random_class = random.choice(
                [idx for idx in list(self.sketches_dict.keys()) if idx != image_class]
            )
...
class AugmentedContrastiveDataset(ContrastiveDataset):
    ...
    def __getitem__(self, index: int):
        ...
        return (torch.stack((
            self.transform(input[0]), 
            self.transform(input[1]))
            ), 
            target
        )
...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        random_negative_class = random.choice(
            [idx for idx in list(self.sketches_dict.keys()) if idx != image_class]
        )
